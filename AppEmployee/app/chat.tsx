...
// Chat con el empleador de un trabajo.
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: Paleta.fondo }} behavior="padding">
      ...
      <View
        ...
        <Pressable
          onPress={() => (router.canGoBack() ? router.back() : router.replace('/buscar' as any))}
          className="w-10 h-10 rounded-full items-center justify-center active:opacity-70">
          <MaterialIcons name="arrow-back" size={24} color={Paleta.principal} />
        </Pressable>
        <View className="flex-1 ml-1">
          <Text className="text-principal text-base font-nunito-bold">Chat con el empleador</Text>
          {trabajo ? (
        ...
        <View className="px-6 pt-3 bg-white border-t border-neutro" style={{ paddingBottom: insets.bottom + 12 }}>
          <Text className="text-neutro text-[13px] font-nunito text-center leading-5">
            {trabajo.estado === 'completado'
              ...
              : 'Vas a poder escribir cuando arranque el trabajo, después de validar el PIN.'}
          </Text>
        </View>
      ) : null}
    </KeyboardAvoidingView>
