...
- **Root (`/src`, `/prisma`, `/test`)** — NestJS backend, referred to in code comments as "el backend de Nacho/Ignacio". Originally handled identity verification and location/distance features; **as of 2026-07, both apps no longer call it for those** — that functionality was reimplemented in `backend/` (see below). It still owns matching, push notifications, job evidence photos and ratings: `backend/` calls it server-to-server (`NACHO_API_URL`), and the apps call it directly only to register push tokens and upload/list evidence photos (see "Expo apps" below).
- **`backend/`** — Express/CommonJS backend ("el backend de Nico"). Handles auth, users, the jobs (`trabajos`) lifecycle including chat and rating, **and now also identity verification and location/distance** (ported from the NestJS project above). This is the main backend both Expo apps talk to. See [backend/CLAUDE.md](backend/CLAUDE.md) for its full architecture — read that file before working in `backend/`.
